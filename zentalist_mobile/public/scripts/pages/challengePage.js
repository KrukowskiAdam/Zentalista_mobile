...
export async function initChallengePageNew() {
 ...
 if (container) {
 container.innerHTML = `
 <div class="text-center py-8 bg-learning-app-design-1 rounded-lg">
 ...
 container.innerHTML = `
 <div class="text-center py-8 bg-learning-app-design-1 rounded-lg">
 Could not sync challenge categories for selected language.
 </div>
 `;
 }
 return;
...
function renderCategorySelection(categories) {
 ...
 <p class="text-xs uppercase tracking-wide">Complete category to retry</p>
 `;
...
function renderPendingRetryCard(category, source, language, status) {
 ...
 return `
 <div class="challenge-category-card bg-learning-app-design-1 rounded-lg p-4 opacity-75">
 ...
 <p class="text-xs uppercase tracking-wide mb-1">${mainCategoryTitle}</p>
 <h4 class="font-semibold">${categoryName}</h4>
 <div class="flex items-center gap-2 mt-1">
 <div class="w-20 bg-learning-app-design-3 rounded-full h-1.5">
 <div class="bg-level-learning h-1.5 rounded-full" style="width: ${progressPercent}%"></div>
 </div>
 <span class="text-xs">${progressPercent}%</span>
 </div>
 </div>
 ...
 <a href="/learn?source=${source}&category=${encodeURIComponent(categoryName)}&lang=${language}" class="touch-target inline-flex w-full sm:w-auto items-center justify-center rounded-lg bg-learning-app-design-4 px-4 py-2 text-sm font-medium transition-colors hover:bg-learning-app-design-1">
 Continue
 </a>
 </div>
 </div>
...
function renderLanguageHeader(langDetails) {
 ...
 <span class="inline-flex items-center justify-center w-10 h-10 rounded-full overflow-hidden bg-learning-app-design-3">
 ${flag}
 </span>
 <div>
...
function renderCategoryCard(
 ...
 buttonClass = 'inline-flex items-center justify-center rounded-lg bg-level-expert px-4 py-2 text-sm font-semibold transition-colors hover:bg-level-expert-dark';
 } else if (isSuccess) {
 buttonClass = 'inline-flex items-center justify-center rounded-lg bg-level-learning px-4 py-2 text-sm font-semibold transition-colors hover:bg-level-learning';
 } else {
 buttonClass = 'inline-flex items-center justify-center rounded-lg bg-level-expert px-4 py-2 text-sm font-semibold transition-colors hover:bg-level-expert-dark';
 }
 } else if (awaitingAuth) {
 buttonClass = 'inline-flex items-center justify-center rounded-lg bg-learning-app-design-4 px-4 py-2 text-sm font-semibold cursor-wait';
 } else if (loggedIn) {
 buttonClass = 'btn btn-disabled btn-sm';
 } else {
 // Not logged in - show disabled style
 buttonClass = 'inline-flex items-center justify-center rounded-lg bg-learning-app-design-4 px-4 py-2 text-sm font-semibold cursor-not-allowed opacity-60';
 }
 ...
 return `
 <div class="challenge-category-card bg-learning-app-design-1 rounded-lg p-4 hover:shadow-md transition-shadow">
 <div class="challenge-card-layout flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
 <div class="challenge-card-info flex-1 w-full">
 <p class="text-xs uppercase tracking-wide mb-1">${mainCategoryTitle}</p>
 <h4 class="font-semibold">${categoryName}</h4>
 ...
 <div class="challenge-card-actions flex w-full sm:w-auto items-center gap-3">
 ${statusBadge}
 <button class="start-challenge-btn touch-target w-full sm:w-auto ${buttonClass}" data-source="${source}"
...
function startChallenge(source, categoryName, language) {
 ...
 if (!languageBundle) {
 console.warn('Language data is still loading. Please try again in a moment.');
 return;
 }
 ...
 console.warn('Selected category was not found. Please refresh this page.');
 return;
 }
...
function renderQuestion() {
 ...
 document.getElementById('quiz-progress').textContent = `${progress.current}/${progress.total}`;
 document.getElementById('quiz-progress-bar').style.width = `${progress.percentage}%`;
 // Update category name
 if (challengeService.currentChallenge) {
 document.getElementById('quiz-category-name').textContent = challengeService.currentChallenge.categoryName;
 }
...
function showResults() {
 ...
 const pointsMessage = results.bonusPoints > 0 ? `You earned +${results.bonusPoints} points!`
 : 'No more points available for this category.';
 messageElement.innerHTML = `
 <div class="bg-learning-app-design-2 p-4">
 <p class="">
 <strong>Category Mastered!</strong> ${pointsMessage}
 <br><span class="text-sm">You can play again for practice, but points are capped.</span>
 </p>
 </div>
 `;
 } else {
 ...
 messageElement.innerHTML = `
 <div class="bg-learning-app-design-5 p-4">
 <p class="">
 <strong>Great job!</strong> ${pointsInfo}
 <br><span class="text-sm">You can replay anytime, but points are capped after the first success.</span>
 </p>
 </div>
 `;
 }
 ...
 scoreElement.textContent = `Score: ${results.score}% (${results.correctAnswers}/${results.totalQuestions})`;
 messageElement.innerHTML = `
 <div class="bg-learning-app-design-1 p-4">
 <p class="">
