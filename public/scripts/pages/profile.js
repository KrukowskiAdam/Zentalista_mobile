...
class ProfileManager {
  ...
  showPageStatus(message, tone = "info") {
    ...
    this.elements.pageStatus.className = `mb-4 rounded-lg border px-4 py-3 text-sm ${toneClass}`;
  ...
  updateAccountTypeUI(isPremium, userData, user) {
    ...
    if (isPremium) {
      ...
      this.elements.accountType.className = "font-medium text-yellow-400";
      this.elements.trialInfoRow?.classList.add("hidden");
    } else {
      // Free user - check trial
      this.elements.accountType.textContent = "Free";
      this.elements.accountType.className = "font-medium text-gray-400";
      ...
      if (effectiveStartDate) {
        ...
        if (msLeft > 0) {
          ...
          this.elements.trialStatus.className = "font-medium text-green-400";
        } else {
          // Trial expired
          this.elements.trialInfoRow?.classList.remove("hidden");
          this.elements.trialStatus.textContent = "Expired";
          this.elements.trialStatus.className = "font-medium text-red-400";
        }
      } else {
