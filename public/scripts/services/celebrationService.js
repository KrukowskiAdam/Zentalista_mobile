...
class CelebrationService {
  ...
  ensureModal() {
    ...
    overlay.className = "fixed inset-0 z-[120] bg-black/80 backdrop-blur-sm flex items-center justify-center px-4 hidden";
    ...
    card.className = "relative w-full max-w-md bg-dark-card border border-dark-border rounded-2xl shadow-2xl p-6 text-center z-10";

    const closeBtn = document.createElement("button");
    closeBtn.type = "button";
    closeBtn.className = "absolute top-4 right-4 text-gray-500 hover:text-gray-200 transition-colors";
    ...
    this.titleElement.className = "text-2xl font-bold text-white mb-2";

    this.subtitleElement = document.createElement("p");
    this.subtitleElement.className = "text-sm text-gray-400 mb-6";
    ...
    this.challengeButton.className = "w-full rounded-xl border border-level-expert text-level-expert text-sm font-semibold py-2 transition duration-150 hover:bg-level-expert hover:text-white focus:outline-none";
    ...
    maybeLaterButton.className = "w-full rounded-xl text-gray-400 text-sm font-medium py-2 hover:text-gray-200 focus:outline-none";
  ...
  injectFallbackStyles() {
    ...
    style.textContent = `
      ...
      .celebration-confetti-piece {
        ...
        background-color: var(--confetti-color, #ff8c02);
        border-radius: 999px;
        opacity: 0;
  ...
  playFallbackConfetti() {
    ...
    const colors = ["#FF8C02", "#05C7F2", "#04BF33", "#E2E8F0", "#F472B6"];
    const pieceCount = 28;
  ...
  updateChallengeButtonState(loggedIn) {
    ...
    if (loggedIn) {
      this.challengeButton.textContent = "Take the challenge";
      this.challengeButton.classList.remove("bg-amber-500/20", "text-amber-200");
      this.challengeButton.classList.add("text-level-expert");
    } else {
      this.challengeButton.textContent = "Log in to take the challenge";
      this.challengeButton.classList.add("bg-amber-500/20", "text-amber-200");
      this.challengeButton.classList.remove("text-level-expert");
    }
  }
