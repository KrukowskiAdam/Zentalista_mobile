...
export class ButtonComponent {
  ...
  createWordButton(word, cardId, cardWords = []) {
    ...
    const buttonClass = `fc-button min-w-[60px] transition-all duration-300 ease-in-out min-h-[80px] leading-[1.2] flex items-center justify-center text-white ${
      isWordLocked ? CONFIG.cards.buttonColors.locked : "bg-dark-card"
    ...
    if (isWordAccessible) {
      ...
      if (romanization) {
        primaryLangText.innerHTML = `
          ...
          <div class="text-lg text-gray-600 mt-2">${romanization}</div>
        `;
      } else {
        primaryLangText.textContent = mainText;
      }
