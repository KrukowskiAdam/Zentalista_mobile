...
export class SidebarComponent {
  ...
  buildCategorySection(parent, title, categories, isPremium = false) {
    ...
    const accordionDiv = createElement('div', {
      parent,
      className: 'collapse collapse-arrow join-item border-dark-border border bg-dark-card'
    });
    ...
    const accordionTitle = createElement('div', {
      parent: accordionDiv,
      className: 'collapse-title text-base font-semibold text-gray-200',
      text: title
    });
    ...
    filteredCategories.forEach(category => {
      ...
      const link = createElement('a', {
        parent: item,
        className: `text-sm py-2 px-4 rounded-lg hover:bg-dark-border transition-colors ${
          stateService.categoryFilter === category.name ? 'bg-level-expert text-level-expert hover:bg-level-expert' : 'text-gray-300'
  ...
  handleSidebarNavigation(targetCategoryInfo, targetSection, firstSection, categoryData) {
    if (targetCategoryInfo && targetSection) {
      ...
      setTimeout(() => {
        ...
        window.dispatchEvent(new CustomEvent('categoryChanged', { 
          detail: { category: targetCategoryInfo.category } 
        }));
      }, 100);
    } else {
      // Default behavior
  ...
  updateCategorySidebar() {
    ...
    this.updateSidebarTimeout = setTimeout(() => {
      ...
      document.querySelectorAll('#category-sidebar-menu a').forEach(link => {
        ...
        if (isActive) {
          link.className = 'text-sm py-2 px-4 rounded-lg transition-colors bg-level-expert text-level-expert hover:bg-level-expert';
          ...
          link.className = `text-sm py-2 px-4 rounded-lg hover:bg-dark-border transition-colors text-gray-300 ${isLocked ? 'opacity-60 cursor-not-allowed' : ''}`;
        }
      });
